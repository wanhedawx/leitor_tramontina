...
st.markdown(
    ...
    /* Inverte a cor se o tema do navegador for CLARO */
...
# --- 5. ENTRADA DE DADOS (VERTICAL) ---
# Removi as colunas para voltarem a ficar um embaixo do outro
...
if st.button("🚀 Processar Pedidos", use_container_width=True, type="primary") and arquivos and sel_cliente:
    todos_pedidos = []
    ...
    # --- OPÇÕES DE DOWNLOAD CONSOLIDADO ---
    if todos_pedidos:
        st.write("---")
        df_consolidado = pd.concat(todos_pedidos, ignore_index=True)
        st.subheader("📦 Download Consolidado")
